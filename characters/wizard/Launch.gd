...
func is_usable():
	...
	if obj:
		...
		if !redirect and in_flight:
			return false
		# A perfect parry or push-block consumes the wizard's right to
		# redirect — both are defensive resource wins, the wizard shouldn't
		# get a do-over by steering through them. Regular block is fine and
		# stays usable.
